...
class AddAProtectorController @Inject() (
  ...
  private def trustHasProtectorAnswer(draftId: String): RequiredAnswerAction[Boolean] =
    requiredAnswer(
      RequiredAnswer(TrustHasProtectorYesNoPage, routes.TrustHasProtectorYesNoController.onPageLoad(draftId))
    )
  ...
  def onPageLoad(draftId: String): Action[AnyContent] =
    standardActionSets.identifiedUserWithData(draftId).andThen(trustHasProtectorAnswer(draftId)).async {
      implicit request =>
        for {
  ...
  def submitOne(draftId: String): Action[AnyContent] =
    standardActionSets.identifiedUserWithData(draftId).andThen(trustHasProtectorAnswer(draftId)).async {
      implicit request =>
        yesNoForm
          .bindFromRequest()
  ...
  def submitAnother(draftId: String): Action[AnyContent] =
    standardActionSets.identifiedUserWithData(draftId).andThen(trustHasProtectorAnswer(draftId)).async {
      implicit request =>
        addAnotherForm
          .bindFromRequest()
  ...
  def submitComplete(draftId: String): Action[AnyContent] =
    standardActionSets.identifiedUserWithData(draftId).andThen(trustHasProtectorAnswer(draftId)).async {
      implicit request =>
  ...
  private def setTaskStatus(userAnswers: UserAnswers, draftId: String, selection: AddAProtector)(implicit
    hc: HeaderCarrier
  ): Future[HttpResponse] = {
