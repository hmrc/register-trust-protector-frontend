...
case class Protectors(
  individuals: List[IndividualProtectorViewModel] = Nil,
  ...
  val maxedOutOptions: List[RadioOption] =
    options.filter(_._1 >= MAX).map { x =>
      RadioOption(IndividualOrBusinessToAdd.prefix, x._2.toString)
    }
