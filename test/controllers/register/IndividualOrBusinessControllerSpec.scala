...
class IndividualOrBusinessControllerSpec extends SpecBase with MockitoSugar {
  ...
  "IndividualOrBusiness Controller" must {
    ...
    "populate the view without the previous answer when the question has previously been answered" in {
      ...
      val answers = emptyUserAnswers.set(IndividualOrBusinessPage, individualOrBusinessAnswer).success.value
    ...
    "redirect to the Name page when Company is selected" in {
      ...
      val request =
        FakeRequest(POST, individualOrBusinessRoute)
          .withFormUrlEncodedBody(("value", individualOrBusinessAnswer.toString))
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      val request = FakeRequest(POST, individualOrBusinessRoute)
