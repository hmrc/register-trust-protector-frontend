...
import pages.register.individual.{NamePage, NonUkAddressPage}
...
class NonUkAddressControllerSpec extends SpecBase with MockitoSugar {
  ...
  "NonUkAddress Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val answers = baseAnswers.set(NonUkAddressPage(index), answer).success.value
      ...
      contentAsString(result) mustEqual
        view(form.fill(answer), countryOptions, name.toString, index, draftId)(fakeRequest, messages).toString
