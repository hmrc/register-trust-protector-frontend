...
import pages.register.individual.mld5.{CountryOfResidenceInTheUkYesNoPage, CountryOfResidencePage, CountryOfResidenceYesNoPage}
    ...
    "Yes selected - set CountryOfResidencePage to 'GB' " in {
      forAll(arbitrary[UserAnswers]) {
        initial =>
          ...
          result.get(CountryOfResidenceInTheUkYesNoPage(0)).get mustBe true
      }
    }
    ...
    "No selected" in {
      forAll(arbitrary[UserAnswers]) {
        initial =>
          ...
          result.get(CountryOfResidenceInTheUkYesNoPage(0)).get mustBe false
      }
    }
