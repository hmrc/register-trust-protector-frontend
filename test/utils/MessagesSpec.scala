...
class MessagesSpec extends SpecBase {
  ...
  override lazy val fakeApplication: Application = new GuiceApplicationBuilder()
    .configure(
      defaultAppConfigurations ++ Seq("application.langs" -> "en,cy", "features.welsh-language-support" -> true): _*
    )
    .build()
  ...
  "All message files" should {
    ...
    "have a resolvable message for keys which take args" in {
      ...
      missingFromEnglish foreach { messageKey =>
        println(s"Key which has arguments in English but not in Welsh: $messageKey")
      }
      missingFromWelsh foreach { messageKey =>
        println(s"Key which has arguments in Welsh but not in English: $messageKey")
      }
    ...
    "have the same args in the same order for all keys which take args" in {
      ...
      mismatchedArgSequences foreach {
        case (messageKey, engArgSeq, welshArgSeq) =>
          println(
            s"messageKey which has different arguments or order of arguments between English and Welsh: " +
              s"$messageKey -- English arg seq=$engArgSeq and Welsh arg seq=$welshArgSeq")
      }
